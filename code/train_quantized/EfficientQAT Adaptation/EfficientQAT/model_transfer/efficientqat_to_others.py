...
def main():
    ...
    # Manually add quantization config to the model's config file
    ...
    if args.target_format == 'gptq':
        ...
        model = GPTQModel.from_quantized(args.model, device_map='auto',torch_dtype=torch.float16, quantize_config=quant_config,backend=BACKEND.TRITON)

    elif args.target_format == 'torch':
        model = GPTQModel.from_quantized(args.model, device_map='auto',torch_dtype=torch.float16, quantize_config=quant_config,backend=BACKEND.TORCH)

    elif args.target_format == 'bitblas':
        # take a lone time for the first time runing
        try:
            model = GPTQModel.from_quantized(args.model, device_map='auto',torch_dtype=torch.float16, quantize_config=quant_config,backend=BACKEND.BITBLAS)
            args.eval_ppl = False # BitBLAS have bug, which should re-load model for evaluation otherwise would cause wrong outputs
        except:
            model = GPTQModel.from_quantized(args.model, device_map='auto',torch_dtype=torch.float16, backend=BACKEND.BITBLAS)
    else:
