...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    run_custom_test: bool = field(
        default=False,
        metadata={"help": "Whether to run the custom NLP-for-Education test."}
    )
...
def train():
    ...
    if args.run_custom_test:
        logger.info("*** Custom NLP-for-Education test ***")
        from datasets import load_dataset

        # Load the held-out split
        test_ds = load_dataset(
            "brygotti/NLP4Education_english_single_mcq_4_choices",
            split="test"
        )

        # Evaluate – we reuse the same evaluate_mmlu routine
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        eval_metrics = evaluate_mmlu(
            dataset=test_ds,
            model=model,
            tokenizer=tokenizer,
            device=device,
            harness_fn=mmlu_harness_hf
        )

        with open(os.path.join(args.output_dir, "custom_test_results.json"), "w") as fout:
            json.dump(eval_metrics, fout, indent=2)
