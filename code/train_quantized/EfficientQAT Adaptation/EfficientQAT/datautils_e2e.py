...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        ...
        if dataset_name == 'mnlp':
            dataset_name = "TheS3b/unified-dataset-filtered-430K"
            dataset = load_dataset(dataset_name)
            dataset = dataset.filter(lambda x: x['relevance1'] > 0.32 and x['relevance2'] > 0.32)
    ...
    def format_dataset(dataset, dataset_format):
        if (
            ...
            dataset_format == 'mnlp' or
            (dataset_format is None and args.dataset == 'mnlp')):

            dataset = dataset.map(lambda x: {
                'input': x['prompt'],
                'output': x['completion'],
            }, remove_columns=[col for col in dataset['train'].column_names if col not in ['prompt', 'completion']])
