...
def update_dataset(model, layer, dataset, dev, attention_mask, position_ids):
    with torch.no_grad():
        with torch.amp.autocast(device_type="cuda"):
            for index, inps in enumerate(dataset):
                ...
                rotary_emb = model.rotary_emb(inps, position_ids=position_ids)

                new_data = layer(inps, attention_mask=attention_mask,position_ids=position_ids, position_embeddings=rotary_emb)[0].to('cpu')
...
def block_ap(
    ...
    if hasattr(model.model, 'rotary_emb'):
        # for llama-3.1
        print("Moving rotary embedding to device")
    ...
    print("-- Step 3.1: Catching inputs of training set --")
    ...
    print("-- Step 3.2: Catching inputs of validation set --")
    ...
    print("-- Step 4: Move embedding layer and first layer to CPU --")
    ...
    print("-- Step 5: Copy fp input as the quant input --")
    if args.off_load_to_disk:
    ...
    print(f"-- Step 6: Start training ({len(layers)} blocks) --")
    ...
    for block_index in range(len(layers)):
        ...
        if args.epochs > 0:
            update_dataset(model.model, qlayer,fp_train_inps,dev,attention_mask,position_ids)
            update_dataset(model.model, qlayer,fp_val_inps,dev,attention_mask,position_ids)
            ...
            for epoch in range(args.epochs):
                ...
                for index, (quant_inps, fp_inps) in enumerate(zip(quant_train_inps, fp_train_inps)):    
                    ...
                    with torch.amp.autocast(device_type="cuda"):
                        ...
                        rotary_emb = model.model.rotary_emb(input, position_ids=position_ids)
                        quant_out = qlayer(input, attention_mask=attention_mask_batch,position_ids=position_ids, position_embeddings=rotary_emb)[0]
        ...
        if args.epochs>0:
            update_dataset(model.model, qlayer,quant_train_inps,dev,attention_mask,position_ids)
            update_dataset(model.model, qlayer,quant_val_inps,dev,attention_mask,position_ids)
